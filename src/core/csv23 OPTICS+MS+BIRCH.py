...
from bin.params_algo_csv23 import optics_params, meanshift_params, birch_params

def main():
    output_dir = "../../results/csv23"
    os.makedirs(output_dir, exist_ok=True)
    today_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_path = os.path.join(output_dir, f"risultati_cluster_csv23_{today_str}.txt")

    with open(output_path, "w", encoding="utf-8") as f, redirect_stdout(f):
        # Caricamento e pulizia del dataset
        df = pd.read_csv('../../data/raw/journal.pone.0158570_S2File_depression_heart_failure_v2.csv')
