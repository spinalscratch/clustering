...
from bin.params_algo_csv66 import optics_params, meanshift_params, birch_params

def main():
    output_dir = "../../results/csv66"
    os.makedirs(output_dir, exist_ok=True)
    today_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    output_path = os.path.join(output_dir, f"risultati_cluster_csv66_{today_str}.txt")

    with open(output_path, "w", encoding="utf-8") as f, redirect_stdout(f):
        # Caricamento e pulizia del dataset
        df = pd.read_csv('../../data/raw/journal.pone.0148699_S1_Text_Sepsis_SIRS_EDITED.csv')
